...
(async()=>{
  ...
  try{
    ...
    await page.locator('#supervisionEvaluationsPanel').getByLabel('Residente',{exact:true}).selectOption({label:'Residente Alfa · R1'});
    await page.locator('#supervisionEvaluationsPanel').getByLabel('Preceptor',{exact:true}).selectOption({label:'Preceptor Dois'});assert.equal(await page.locator('.supervision-evaluation').count(),1);
    ...
    await page.locator('#supervisionEvaluationsPanel').getByLabel('Ano da residência',{exact:true}).selectOption('R2');await page.locator('#supervisionEvaluationsPanel').getByLabel('Módulo',{exact:true}).selectOption('Neonatologia');await page.locator('#supervisionEvaluationsPanel').getByLabel('Período avaliado',{exact:true}).selectOption('Agosto');assert.equal(await page.locator('.supervision-evaluation').count(),1);
    await page.locator('#supervisionEvaluationsPanel').getByLabel('Avaliações desde',{exact:true}).fill('2026-09-01');assert.equal(await page.locator('.supervision-evaluation').count(),0);
    await page.locator('#supervisionEvaluationsPanel').getByLabel('Até',{exact:true}).fill('2026-08-01');assert.match(await page.locator('#supervisionSummary').innerText(),/data inicial/);
    await page.getByRole('button',{name:'Limpar filtros',exact:true}).click();
